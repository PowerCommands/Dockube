...
namespace PainKiller.CodeGeneratorCommands.Commands;
...
public class ApiCommand : CommandWithToolbarBase<PowerCommandsConfiguration>
{
    public ApiCommand(string identifier, PowerCommandsConfiguration configuration) : base(identifier, configuration, autoShowToolbar: false) { }
    protected override void ReadLineService_CmdLineTextChanged(object? sender, CmdLineTextChangedArgs e)
    {
        ...
        else if(LatestHighlightedCommand == Identifier)
        {
            ClearToolbar();
            Labels.Clear();
        }
    ...
    {
        if(Input.SingleArgument == "generate") GenerateCode();
        return Ok();
    }
